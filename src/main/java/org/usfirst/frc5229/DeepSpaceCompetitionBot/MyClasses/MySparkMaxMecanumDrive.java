...
package org.usfirst.frc5229.DeepSpaceCompetitionBot.MyClasses;
 ...
 * <br>{@link #driveCartesian(double, double, double, double, double)} is equivalent to
 * {@link edu.wpi.first.wpilibj.RobotDrive#mecanumDrive_Cartesian(double, double, double, double, double)}
 * if a deadband of 0 is used, and the ySpeed and gyroAngle values are inverted compared to
 * RobotDrive (eg driveCartesian(xSpeed, -ySpeed, zRotation, -gyroAngle, maxVelocity).
 * <br>{@link #drivePolar(double, double, double, double)} is equivalent to
 * {@link edu.wpi.first.wpilibj.RobotDrive#mecanumDrive_Polar(double, double, double, double)} if a
 * deadband of 0 is used.
 */
public class MySparkMaxMecanumDrive extends RobotDriveBase {
  ...
  private final CANPIDController c_frontLeftController;
  private final CANPIDController c_rearLeftController;
  private final CANPIDController c_frontRightController;
  private final CANPIDController c_rearRightController;
   ...
   */
  public MySparkMaxMecanumDrive(SpeedController frontLeftMotor, SpeedController rearLeftMotor,
                      SpeedController frontRightMotor, SpeedController rearRightMotor,
                      CANPIDController frontLeftController, CANPIDController rearLeftController,
                      CANPIDController frontRighController, CANPIDController rearRightController) {

    verify(frontLeftMotor, rearLeftMotor, frontRightMotor, rearRightMotor, 
      frontLeftController, rearLeftController, frontRighController, rearRightController);
    ...
    c_frontLeftController = frontLeftController;
    c_rearLeftController = rearLeftController;
    c_frontRightController = frontRighController;
    c_rearRightController = rearRightController;

    addChild(c_frontLeftController);
    addChild(c_rearLeftController);
    addChild(c_frontRightController);
    addChild(c_rearRightController);
  }

  /**
   * Verifies that all motors and PID controllers are nonnull, throwing a NullPointerException if any of them are.
  ...
  private void verify(SpeedController frontLeftMotor, SpeedController rearLeftMotor,
                      SpeedController frontRightMotor, SpeedController rearRightMotor,
                      CANPIDController frontLeftController, CANPIDController rearLeftController,
                      CANPIDController frontRighController, CANPIDController rearRightController) {

    if (frontLeftMotor != null && rearLeftMotor != null && frontRightMotor != null && rearRightMotor != null &&
      frontLeftController != null && rearLeftController != null && frontRighController != null && rearRightController != null) {
      return;
    }
    ...
    if (frontLeftController == null) {
      joiner.add("frontLeftController");
    }
    if (rearLeftController == null) {
      joiner.add("rearLeftController");
    }
    if (frontRighController == null) {
      joiner.add("frontRightController");
    }
    if (rearRightController == null) {
      joiner.add("rearRightController");
    }
   ...
   *                    positive.
   * @param maxVelocity The robot's max target velocity.
   */
  @SuppressWarnings("ParameterName")
  public void driveCartesian(double ySpeed, double xSpeed, double zRotation, double maxVelocity) {
    driveCartesian(ySpeed, xSpeed, zRotation, 0.0, maxVelocity);
  }
   ...
   */
  @SuppressWarnings("ParameterName")
  public void driveCartesian(double ySpeed, double xSpeed, double zRotation, double gyroAngle, double maxVelocity) {
    if (!m_reported) {
    ...
    c_frontLeftController.setReference((wheelSpeeds[MotorType.kFrontLeft.value] * m_maxOutput * maxVelocity), ControlType.kVelocity);
    c_frontRightController.setReference((wheelSpeeds[MotorType.kFrontRight.value] * m_maxOutput * m_rightSideInvertMultiplier * maxVelocity), ControlType.kVelocity);
    c_rearLeftController.setReference((wheelSpeeds[MotorType.kRearLeft.value] * m_maxOutput *maxVelocity), ControlType.kVelocity);
    c_rearRightController.setReference((wheelSpeeds[MotorType.kRearRight.value] * m_maxOutput * m_rightSideInvertMultiplier * maxVelocity), ControlType.kVelocity);
   ...
   */
  @SuppressWarnings("ParameterName")
  public void drivePolar(double magnitude, double angle, double zRotation, double maxVelocity) {
    if (!m_reported) {
    ...
    driveCartesian(magnitude * Math.sin(angle * (Math.PI / 180.0)),
                   magnitude * Math.cos(angle * (Math.PI / 180.0)), zRotation, 0.0, maxVelocity);
  }
